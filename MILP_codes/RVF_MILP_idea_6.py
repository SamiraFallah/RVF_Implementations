...
def convertWeakToStrongNDP(_totalVarsInit, _print=False):
    ...
    if _print and debug_print:
        with open("Results_MILP_details_idea_6_{}.txt".format(instanceName), "a") as _filed:
...
start = time.time()     
while True:
#     print('shouldCallSublist', shouldCallSublist)
#     print('totalIter', totalIter)
    ...
    if (status in [3, 4]) or elapsedTime > timeLimit:
        ...
        if status in [3, 4]:
            with open("Results_MILP_idea_6_{}.txt".format(instanceName), "w") as _file:
    ...
    if (int_part in allIntPartDict.values()):
