...
# from InstanceTest.obj_2_var_10_nzr_4_intR_6_U_1_ins_0 import *
...
def convertWeakToStrongNDP_one_stage(_totalVarsInit, _print=False):
    ...
    if _print and debug_print:
        with open("Results_MILP_details_idea_3_{}.txt".format(instanceName), "a") as _filed:
...
while True:
    ...
    if status in [3, 4] or elapsedTime > timeLimit:
        ...
        if status in [3, 4]:
            with open("Results_MILP_idea_3_{}.txt".format(instanceName), "w") as _file:
...
# total_time = end_before - start
# iter_time_lst = []
# iter_time_lst.append(total_time)
...
# num_feas_sol = 1
# m.params.SolutionLimit = num_feas_sol
...
#     iter_time_lst.append(iter_time)
# #     print('total', sum(iter_time_lst))
#     end = time.time()
#     elapsedTime = end - start
...
#         if conversionOld:
#             total_part = convertWeakToStrongNDP_one_stage({**dict((k, round(changeValue(v.Xn))) for k,v in intVars.items()), **dict((k, round(changeValue(v.X))) for k,v in conVars.items())}, _print=False)
#         else:
#             total_part = convertWeakToStrongNDP_one_stage({**dict((k, round(changeValue(v.Xn))) for k,v in intVars.items()), **dict((k, round(changeValue(v.X))) for k,v in conVars.items())}, _print=False)
...
#         else:
#             thisNDP = thisNDP + (round(sum(OBJ[j]*total_part[j] for j in INTVARS)) +
#                            round(sum(OBJ[j]*total_part[j] for j in CONVARS), 2),)
    
#             for k in CONSVARRHS:
#                 thisNDP = thisNDP + (round(sum(MAT[(k, l)]*total_part[l] for l in INTVARS)) +
