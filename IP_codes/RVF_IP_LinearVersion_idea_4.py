...
def convertWeakToStrongNDP(_intVarsInit, _print=False):
    ...
    if _print and debug_print:
        with open("Results_IP_Linear_details_idea_4_{}.txt".format(instance.split('.')[0]), "a") as _filed:
...
while True:
    ...
    if (thetaVar.X < 0.1 and allIntPart) or elapsedTime > timeLimit:
        if thetaVar.X < 0.1:
            print("Finished!")
        else:
        ...
        if thetaVar.X < 0.1:
            with open("Results_IP_Linear_idea_4_{}.txt".format(instance.split('.')[0]), "w") as _file:
